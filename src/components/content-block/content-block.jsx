...
const articleComponents = {
  h2({ node, children, ...props }) {
    ...
    return (
      <h2 id={id} tabIndex="-1" className="scroll-mt-16 flex gap-x-2 items-center mb-6 mt-24 font-bold">
        <span>{children}</span>
        <a
          href={`#${id}`}
          title="permalink"
  ...
  h3({ node, children, ...props }) {
    ...
    return (
      <h3 id={id} tabIndex="-1" className="scroll-mt-16 flex gap-x-2 items-center mb-2 mt-12 font-bold">
        <span>{children}</span>
        <a
          href={`#${id}`}
          title="permalink"
  ...
  pre({ node, ...props }) {
    return (
      <pre className="mb-6 group lg:rounded-lg bg-[var(--gray-8)] p-4 m-0 max-w-[100vw] bleed overflow-auto transform relative left-1/2 -translate-x-1/2">
        <button data-copy-code="true" data-copied="false" className="absolute right-1 top-1 group-hover:opacity-100 transition opacity-0 bg-surface-4 rounded-md w-11 h-11 grid place-items-center">
