 ...
 * */
import fs from 'fs'
const filePath = `${process.cwd()}/src/data/content.json`
...
for (const item of JSON.parse(content).content) {
  if (item.category === type) {
    posts.push({
      ...
      links: item.links.map(link => ({ ...link, url: link.link }))
    })
  }
}

const destPath = `${process.cwd()}/src/data/${type}.json`
