      ...
      if (node.value.indexOf('<TableOfContents') !== -1) {
        ...
        const children = headers
          .map((h) => {
            ...
            return `
              <li class="uppercase text-fluid--1 pl-1 ${h.depth > 2 ? 'ml-6' : 'ml-2'}">
                <a href="#${id}" className="text-text-1 block w-full decoration-brand-stroke">${h.children[0].value}</a>
              </li>`
          })
          .join('')
        node.value = `
...
const defaultComponents = {
  aside({ node, dataType, ...props }) {
    if (node?.properties?.dataType === 'tableofcontents') {
      return <aside className="my-12" {...props}>{props.children}</aside>
    }
    return <aside className="my-12 rounded-lg bg-surface-3 text-text-2 p-8" {...props}>{props.children}</aside>
  },
  hr({ node, ...props }) {
  ...
  blockquote({ node, ...props }) {
    return <blockquote {...props} className="mb-6 border-l-4 pl-8 border-brand-stroke italic"></blockquote>
  },
  ...
  pre({ node, ...props }) {
    return (
      <pre className="rounded-lg bg-[var(--gray-10)] p-4 m-0 overflow-auto mb-6">
        {props.children}
      </pre>
    )
  },
  ...
  tweet({ node, account = 'jh3yy', id, ...props }) {
    return (
      <>
        <blockquote className="twitter-tweet mb-6" data-theme="dark">
...
const articleComponents = {
  h2({ node, children, ...props }) {
    ...
    return (
      <h2 id={id} tabIndex="-1" className="flex gap-x-2 items-center mb-6 mt-24 font-bold">
        <span>{children}</span>
        <a
          href={id}
          title="permalink"
  ...
  h3({ node, children, ...props }) {
    ...
    return (
      <h3 id={id} tabIndex="-1" className="flex gap-x-2 items-center mb-2 mt-12 font-bold">
        <span>{children}</span>
        <a
          href={id}
          title="permalink"
  ...
  pre({ node, ...props }) {
    return (
      <pre className="mb-6 group lg:rounded-lg bg-[var(--gray-10)] p-4 m-0 max-w-[100vw] bleed overflow-auto transform relative left-1/2 -translate-x-1/2">
        <button data-copy-code="true" className="absolute right-1 top-1 group-hover:opacity-100 transition opacity-0 bg-surface-4 rounded-md w-11 h-11 grid place-items-center">
...
const cardComponents = {
  ...
  pre({ node, ...props }) {
    return (
      <pre className="rounded-lg bg-[var(--gray-10)] p-4 m-0 overflow-auto mb-2">
        {props.children}
      </pre>
    )
  },
